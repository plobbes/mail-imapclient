...
Example:
  ...
  my $hash = $imap->getquota($quotaroot)
    or die "Could not getquota for $quotaroot: $@\n";

The B<getquota> method implements the RFC2087 GETQUOTA command.  It
accepts one "quota root" argument.  If no argument is provided, a
default "quota root" of "user/I<User>" is used.

On error C<undef> is returned, otherwise an ARRAY/ARRAYREF is returned
depending upon LIST/SCALAR context of the caller.  The contents of the
ARRAY should contain L</Results> from the server which should have the
untagged QUOTA response from the server.

See also B<RFC2087>, L</getquotaroot>, L</setquota>, L</quota> and L</quota_usage>.
...
The B<quota> method takes the results from L<getquota> and parses out
the limit returned by the server.  It accepts one "quota root"
argument.  If no argument is provided, a default "quota root" of
"INBOX" is used.

On error C<undef> is returned, otherwise the integer limit provided by
...
The B<quota_usage> method takes the results from L<getquota> and
parses out the usage returned by the server.  It accepts one "quota
root" argument.  If no argument is provided, a default "quota root" of
"INBOX" is used.

On error C<undef> is returned, otherwise the integer usage provided by
the server is returned.
...
Example:
  ...
  my $hash = $imap->setquota( $quotaroot, $resource, $limit )
    or die "Could not setquota for $quotaroot: $@\n";

The B<setquota> method implements the RFC2087 SETQUOTA command.  It
accepts multiple pairs of $resource and $limit arguments.  If no
$quotaroot argument is provided, a default "quota root" of
"user/I<User>" is used.

On error C<undef> is returned, otherwise an ARRAY/ARRAYREF is returned
depending upon LIST/SCALAR context of the caller.  The contents of the
ARRAY should contain L</Results> from the server.
