...
not mean that the argument should be an array reference.  The first
optional argument is always undef as it was deprecated.  The optional
flags and date arguments are handled the same as append_string with
the exception that if date is "1" (one) then the modification time
(mtime) of the file will be used, otherwise the date should be in a
valid RFC3501 date format.
