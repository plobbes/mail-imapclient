...
correspond to the IMAPClient constants C<Connected>,
...
IMAPClient object and its underlying IMAP session. Note that an
IMAPClient object can be in the C<Unconnected> state both before a
...
discussion of the methods available for examining the IMAPClient
...
The Mail::IMAPClient's support for add-on authentication mechanisms is
...
example, F<examples/imtestExample.pl>, can be found in the C<examples>
...
set the Mail::IMAPClient object in sync with its actual status with
...
Mail::IMAPClient object in sync with its actual status. Of these,
...
and C<$imap> is a pointer to the Mail::IMAPClient object. The return
...
would otherwise be performed by Mail::IMAPClient. The replacement
method is called with five arguments. The first is a pointer to the
Mail::IMAPClient object; the rest are the four arguments required by
...
connection. It is called by Mail::IMAPClient with two arguments:
a reference to the Mail::IMAPClient object and the ASCII text
...
If this happens to you, you can always check C<$@>. Mail::IMAPClient
...
The L</new> method creates a new instance of an IMAPClient object. If
...
If the I<Server> parameter is not supplied then the IMAPClient
...
Mail::IMAPClient module, so you typically will only query it and 
...
that are not explicitly supported by Mail::IMAPClient.)
...
Mail::IMAPClient programmers (such as L</delete_message> and all-lowercase 
...
Bush's L<Authen::NTLM>, which can work with Mail::IMAPClient to
...
the B<login> method returns a pointer to the IMAPClient object and
...
IMAPClient object, so a program can re-invoke the L</connect> and
...
Mail::IMAPClient object for both the caller and the reciever then
...
source Mail::IMAPClient object's the L</Buffer> parameter. The higher
...
Mail::IMAPClient object is connected to a server that has the
...
number (or UID). The IMAPClient object must be in a I<Selected>
...
supporting one of the authentication mechanisms that Mail::IMAPClient supports
...
IMAPClient module). Setting I<Clear> to 0 will not cause an
...
value for I<Timeout> will prevent Mail::IMAPClient from blocking in
a read.
...
current status of the IMAPClient object. If invoked with an
...
you are bypassing the Mail::IMAPClient module's L</connect> and/or
...
current status of the IMAPClient object. (Not to be confused with
