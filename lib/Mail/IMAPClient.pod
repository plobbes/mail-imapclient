...
By default a timeout of 0.025 seconds is used, internally the timeout
is passed to L<perlfunc/select>.  The timeout controls how long the
select call blocks if there are no messages waiting to be read from
the server.
